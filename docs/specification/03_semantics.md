...
- Для остальных типов: `return <выражение>;` обязателен на всех путях 
  выполнения
...
- Допустимы два варианта объявления:
  - `fn main(): int` — возвращаемое значение используется как код возврата
  - `fn main(): unit` — программа завершается с кодом 0
