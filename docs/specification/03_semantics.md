...
| `str`   | Неизменяемая Unicode-строка | Последовательность символов |
...
- Все операторы сравнения возвращают значение типа `bool`.
- Неявное преобразование `int -> float` допускается при сравнении числовых типов.

Таблица допустимых типов:

| Типы операндов        | Разрешённые операторы | Результат | Особенности |
|-----------------------|----------------------|-----------|------------|
| `int` с `int`         | `< > <= >= == !=`    | `bool`    | — |
| `float` с `float`     | `< > <= >= == !=`    | `bool`    | Сравнение по IEEE 754 |
| `int` с `float`       | `< > <= >= == !=`    | `bool`    | `int` приводится к `float` |
| `str` с `str`         | `== !=`              | `bool`    | Посимвольное сравнение Unicode |
| `bool` с `bool`       | `== !=`              | `bool`    | — |
| Разные несовместимые типы | —              | —         | Ошибка компиляции |
...
- Инициализация выполняется один раз перед цикл
- Условие проверяется перед каждой итерацией
- Обновление выполняется после тела каждой итерации
- `break` и `continue` работают как в `while`
...
- Для остальных типов: `return <выражение>;` обязателен на всех путях выполнения, за исключением функции `main`
- Для функции `main`: если `return` отсутствует, неявно возвращается `0`
- Нельзя вызывать вне тела функции
...
- Обязательна функция `fn main() : int`
- Без параметров
- Выполнение программы начинается с `main`
- Возвращаемое значение `main` используется как код возврата программы (exit code)
- Если выполнение `main` достигает конца без `return`, неявно возвращается `0`
