...
- `return <выражение>;` обязателен на всех путях выполнения
...
- Точка входа объявляется как `fn main(): int` — возвращаемое значение используется как код возврата
...
| `printf(f: float, precision: int)` | Выводит вещественное число с заданным количеством знаков после запятой |
...
| Отсутствие `return` в функции | `fn foo(): int { }` | Ошибка компиляции |
